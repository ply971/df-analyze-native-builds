...
def evaluate_tuned(
    ...
    if FeatureSelection.Filter in options.feat_select and pred_filtered is not None:
        selections["pred"] = pred_filtered.selected

    embed_models: dict[str, EmbedSelectionModel] = {}
    if FeatureSelection.Embedded in options.feat_select and model_selected.embed_selected is not None:
    ...
    if FeatureSelection.Wrapper in options.feat_select and model_selected.wrap_selected is not None:
